 ...
 *
 * Sprint 5C/6: Reads plan_catalog.json (generated by build_site.py from boats.json)
 ...
 *
 * Stripe price IDs are stored directly in boats.json (stripePriceId field) and
 * flow through to plan_catalog.json at build time. No per-plan env vars needed.
 *
 * Required environment variables (set in Netlify UI):
 *   STRIPE_SECRET_KEY   - sk_live_... or sk_test_... (use test key for testing)
...
exports.handler = async function (event) {
  ...
  // Stripe price ID stored directly in boats.json → plan_catalog.json
  const priceId = plan.stripePriceId;
  if (!priceId) {
    console.error(`No stripePriceId for plan ${planId}`);
    return {
      statusCode: 500,
