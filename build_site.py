...
def build_yacht_page(boat):
    ...
    for key, label in spec_fields:
        val = specs.get(key)
        # Suppress displacement/ballast rows when value is 0 or empty (Sprint QA-SEO F-8)
        if key in ('displacement', 'ballast') and isinstance(val, dict):
            numeric_vals = [v for v in val.values() if isinstance(v, (int, float))]
            if not numeric_vals or all(v == 0 for v in numeric_vals):
                continue
  ...
  <link rel="canonical" href="{SITE_URL}/yacht/{slug}.html" />
...
'''
    ...
    full_html = base_html.replace("{{ pageTitle }} &mdash; Farr Yacht Design", page_title)
    full_html = full_html.replace("{{ content | safe }}", content)
    # Add meta/OG/canonical tags for portfolio page (Sprint QA-SEO)
    portfolio_meta = f'''<meta name="description" content="Explore {total} yacht designs by Farr Yacht Design — racing yachts, superyachts, cruisers, and dinghies spanning six decades of innovation. Search by name, type, decade, or design number." />
  <link rel="canonical" href="{SITE_URL}/portfolio.html" />
  <meta property="og:type" content="website" />
  <meta property="og:url" content="{SITE_URL}/portfolio.html" />
  <meta property="og:title" content="Portfolio — Farr Yacht Design" />
  <meta property="og:description" content="Explore {total} yacht designs by Farr Yacht Design — racing yachts, superyachts, cruisers, and dinghies spanning six decades of innovation." />
  ...
  <meta name="twitter:image" content="{SITE_URL}/images/og-default.png" />'''
    full_html = full_html.replace("{% block head %}{% endblock %}", portfolio_meta)
...
def build_redirects():
    ...
    lines = [
        ...
        "",
        "# Favicon fallback (Sprint QA-SEO F-7)",
...
'''
  ...
  <script>
    ...
    full_html = base_html.replace("{{ pageTitle }} &mdash; Farr Yacht Design", page_title)
    full_html = full_html.replace("{{ content | safe }}", content)
    # Add meta/OG/canonical tags for design-plans page (Sprint QA-SEO)
    dp_meta = f'''<meta name="description" content="Purchase original Farr design drawings — high-resolution PDFs from our archive. Browse available plans, request digitization of archive drawings, or contact us for custom needs." />
  <link rel="canonical" href="{SITE_URL}/design-plans.html" />
  <meta property="og:type" content="website" />
  <meta property="og:url" content="{SITE_URL}/design-plans.html" />
  <meta property="og:title" content="Design Plans — Farr Yacht Design" />
  <meta property="og:description" content="Purchase original Farr design drawings — high-resolution PDFs from our archive. Browse available plans or request digitization." />
  <meta property="og:image" content="{SITE_URL}/images/og-default.png" />
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:image" content="{SITE_URL}/images/og-default.png" />'''
    full_html = full_html.replace("{% block head %}{% endblock %}", dp_meta)
